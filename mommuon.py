...
class MomMuon(torch.optim.Optimizer):
    ...
    """
    def __init__(self, muon_params, lr=0.02, momentum=0.95, beta2=0.995, ns_beta=0.9, ns_every=1, nesterov=True, ns_steps=15,
                 adamw_params=None, adamw_lr=3e-4, adamw_betas=(0.95, 0.95), adamw_eps=1e-8, adamw_wd=0):

        defaults = dict(lr=lr, momentum=momentum, beta2=beta2, nesterov=nesterov, ns_beta=ns_beta, ns_every=ns_every, ns_steps=ns_steps,
        ...
        for group in self.param_groups:
            ...
            for i, p in enumerate(params):
                ...
                if i % self.world_size == self.rank:
                    ...
                    if (group['ns_every'] > 0 and state['step'] % group['ns_every'] == 0) or state['step'] == 0:
                        precond = gradwhiten(g, ns_steps=group['ns_steps'])
                        if "precond_exp_avg" not in state:
                            # i think better to start as the first precond instead of zeros?
                            state["precond_exp_avg"] = precond
                        else:
                            state["precond_exp_avg"].lerp_(precond, 1-group['ns_beta'])

                    g = state["precond_exp_avg"] @ g    
